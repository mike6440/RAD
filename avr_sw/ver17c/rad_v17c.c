...
// v17a 090212 :: baud = 19200, xtal = 7373800
// v17b 090312 :: fix bug in t_end computation
// v17c 090324 :: add correction for the case/dome adc error
/*********************************************
...
//#define XTAL 3686400
#define XTAL 7372800 // v17a
#define BAUD	19200  // v17a default terminal setting
#define OK	1
#define NOTOK	0 
...
float RL10052Temp (unsigned int, unsigned int, float);
...
unsigned    GetNumber(unsigned, float, float, char*);
...
int state;
unsigned char version[10] = "1.17c";  //v15  v16 v17 v17a v17b v17c
unsigned char verdate[20] = "2009,03,24";  //v15 v16 v17 v17a v17b v17c
float Tabs = 273.15;
int adc[NCHANS];   
...
// v17c -- case and dome correction
eeprom float case1 = -7.307e-5;
eeprom float case2 = 0.0827;
eeprom float case3 = -28.76;
eeprom float dome1 = -3.439e-5;
eeprom float dome2 = 0.0414;
eeprom float dome3 = -15.44;
//************ TEST ************************
eeprom unsigned testflag = 0;
...
void main(void)
{
	...
	printf(" Digital Interface Board - Rev C. Feb 2009\r\n");
	...
	while (1) {
		...
		// Note t_end can be = 0
		rtc_get_time(&h,&m,&s);		
		t_end = ((int)m * 60 + (int)s) / looptime; //v17b v16 integer number of loops in current hour.
		t_end = (t_end+1) * looptime;  //  hour seconds to the end of current loop. No overflow.
        //printf("t_end = %d\n", t_end);
		...
		while (1) {
			...
			if( t_now >= t_end || (t_end==3600 && t_now==0) ) break; // v17b probs with clock rollover
			else {
				//PSP THERMOPILE
		...
		ADC2_mV /= nsamps;
		ADC3_mV /= nsamps;
		if ( testflag ) printf("nsamps = %d,  PSP ADC0_mV = %.2f,  PIR ADC1_mV = %.2f\r\n", nsamps, ADC0_mV, ADC1_mV);
		...
		// TCASE COMPUTE -- tcase
		v_case = ADC2_mV - ( case1 * ADC2_mV * ADC2_mV + case2 * ADC2_mV + case3 );  // v17c
        if ( testflag ) printf("CASE ADC2_mV = %.1f,  v_case = %.1f\r\n", ADC2_mV, v_case); 
		therm_circuit_ground(v_case, Cmax, RrefC, Vadc, Vadc, &vt, &Rt, &Pt);   // v17c
		tcase = ysi46000(Rt,Pt);
		
		// TDOME COMPUTE -- tdome
		v_dome = ADC3_mV - ( dome1 * ADC3_mV * ADC3_mV + dome2 * ADC3_mV + dome3 );		// v17c
        if ( testflag ) printf("DOME ADC3_mV = %.1f,  v_dome = %.1f\r\n", ADC3_mV, v_dome); 
		therm_circuit_ground(v_dome, Cmax, RrefD, Vadc, Vadc, &vt, &Rt, &Pt);  // v17c
		...
		printf("$WIR%02d,%02d/%02d/%02d,%02d:%02d:%02d,%4d,%7.1f,%7.2f,%6.2f,%6.2f,%7.2f,%5.1f,%5.1f\r\n", 
...
{ 
	...
	switch (ch) 
	{
        ...
        case '/':
            ...
            printf("'C' -->Set Case %.1f ohms, %.3e, %.3e, %.3e\r\n", RrefC, case1, case2, case3); 
            printf("'D' -->Set Dome %.1f ohms, %.3e, %.3e, %.3e\r\n", RrefD, dome1, dome2, dome3); 
            ...
            printf("'L' -->Set averaging time in seconds. (%d)\r\n", looptime);
            printf(" -----OTHER------------------------------------------------\r\n");
            printf("'S' -->Sample 12 bit A to D.\r\n");
            printf("'r' --> toggle test mode.\r\n");
        ...
        case 'r': 
            ...
            if (testflag) printf("Test mode\r\n");
            else printf("Test mode off\r\n");
            break;
		...
		case 'K' :
			...
			break;
		case 'C' :
			printf("Enter 0 (RefR)   1 (Corr1)  2 (Corr2)   3 (Corr3)\r\n");
			while(1) {
				// CHECK INPUT BUFFER FOR A CHARACTER
				if ( SerByteAvail() )
				{
			...
			// CASE THERMISTOR PARAMETERS
			switch (ch) 
			{
				case '0':
					printf("Change Case Reference Resistor. Current value = %.1f :: ", RrefC);
					if ( GetNumber(12, 5000, 50000, msg) ) { 
						RrefC = atof(msg);
					...
					break;
				case '1':
					printf("Case ADC Correction coefficient 1.  Current value =  %.3e :: ", case1);
					if ( GetNumber(12, -1, 1,msg) ) {
						case1 = atof(msg);
						printf("\r\nCase Correction 1 is now set to %.3e\r\n", case1);
					}
					else {  printf("\r\nOut of Range\r\n"); }
					break;
				case '2':
					printf("Case ADC Correction coefficient 1.  Current value = %.3e :: ", case2);
					if ( GetNumber(12, -1, 1,msg) ) {
						case2 = atof(msg);
						printf("\r\nCase Correction 2 is now set to %.3e\r\n", case2);
					}
					else { printf("\r\nOut of Range\r\n"); }
					break;
				case '3':
					printf("Case ADC Correction coefficient 1.  Current value = %.3e :: ", case3);
					if ( GetNumber(12, -100, 100,msg) ) {
						case3 = atof(msg);
						printf("\r\nCase Correction 3 is now set to %.3e\r\n", case3);
					}
					else { printf("\r\nOut of Range\r\n"); }
					break;
				default: break;
		...
		case 'D' :
			...
			// DOME THERMISTOR PARAMETERS
			switch (ch) 
			{
				case '0':
					printf("Change Dome Reference Resistor. Current value = %.1f :: ", RrefD);
					if ( GetNumber(12, 5000, 50000, msg) ) { 
						RrefD = atof(msg);
					...
					break;
				case '1':
					printf("Dome ADC Correction coefficient 1.  Current value =  %.3e :: ", dome1);
					if ( GetNumber(12, -1, 1,msg) ) {
						dome1 = atof(msg);
						printf("\r\nDome Correction 1 is now set to %.3e\r\n", dome1);
					}
					else {  printf("\r\nOut of Range\r\n"); }
					break;
				case '2':
					printf("Dome ADC Correction coefficient 1.  Current value = %.3e :: ", dome2);
					if ( GetNumber(12, -1, 1,msg) ) {
						dome2 = atof(msg);
						printf("\r\nDome Correction 2 is now set to %.3e\r\n", dome2);
					}
					else { printf("\r\nOut of Range\r\n"); }
					break;
				case '3':
					printf("Dome ADC Correction coefficient 1.  Current value = %.3e :: ", dome3);
					if ( GetNumber(12, -100, 100,msg) ) {
						dome3 = atof(msg);
						printf("\r\nDome Correction 3 is now set to %.3e\r\n", dome3);
					}
					else { printf("\r\nOut of Range\r\n"); }
					break;
				default: break;
		...
		case 'V' :
			...
			printf("Current Thermistor Reference Voltage is: %.4f Volts\r\n", Vtherm);
			printf("Enter New Thermistor Reference Voltage (volots): ");
			for (i=0; i<12; i++)
			{
				msg[i] = getchar();
				printf("%c", msg[i]);
			...
			else 
			{
				Vtherm = atof(msg);
				Vadc = Vtherm;
				printf("\r\nThermistor Reference Voltage is now set to %.4f volts\r\n", Vtherm); 
			}
			break;
		case 'A' :
        case 'a':
...
unsigned GetNumber(unsigned Nchars, float lim1, float lim2, char*msg)
/**************************************************
// READ NUMBERS ENTERED FROM FROM STDIO CHARACTER BY CHARACTER.
// THAT A NUMBER IS ENTERED AND IT IS IN THE LIMITS (lim1,lim2)
// Return 0 if a bad number, 1 if okay.
***************************************************/
{
	unsigned i;
	char c;	
    ...
    //printf("test- msg = %s, float = %.5e\r\n", msg, atof(msg));
	if( atof(msg) < lim1 || atof(msg) > lim2) return(-1);
	else return(1);
}
...
{  
    ...
    //RefVMilliVolts = ( Read_Max186(VREF, 1) ); // v17a use Cmax only
	AVRVMilliVolts = ( Read_Max186(PCTEMP, 1) );
	AVRTemp = RL10052Temp(AVRVMilliVolts, Cmax, 10010);
...
float RL10052Temp (unsigned int c, unsigned int cmax, float Rref)
/***********************************
Calculates Thermistor Temperature
c = A to D count, 0-4095
cmax = max bipolar count, 2048
Rref = reference divider resistor
Modified by rmr 090214 for board 201_uri
v17a
***********************************/
{
	float rt, dum;
    ...
    rt = Rref * (float)c / (float)( cmax*2 - c );
	dum = log(rt/Rref);
