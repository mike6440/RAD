//NOTE: THIS IS THE OFFICIAL ARCHIVE SOURCE CODE THAT RESIDES ON
//   RMR MAC.  ALL OTHER COPIES ARE SUSPECT.  CHECK THE MODIFICATION
//   TIME ON THIS FILE AGAINST ANY COPIES.
// 09,02,14,09,30,53,PST
...
// v17b ITEMS FOR THE NEXT VERSION
//   * Add a menu item like 'S' to show calibrated input voltages.
//   * Reports zeros just before the hour
//   Starting at 59 minutes and 50 seconds after every hour
//   the next 20 or so messages (still at once per 10 seconds)
//   are zero filled. The $ header, the comas and the <cr><lf> are all
//   there, its just that all data in all fields is zeros.
/*********************************************
...
int state;
unsigned char version[10] = "1.17a";  //v15  v16 v17 v17a
unsigned char verdate[20] = "2009,02,12";  //v15 v16 v17 v17a
float Tabs = 273.15;
int adc[NCHANS];   
...
void main(void)
{
	...
	while (1) {
		...
		t_end = (t_end+1) * looptime;  // v17b, hour seconds to the end of current loop. No overflow.
		...
		while (1) {
			...
			if( t_now >= t_end || (t_end==3600 && t_now==0)) break; // v17b get out at clock roll over
			else {
				//PSP THERMOPILE
