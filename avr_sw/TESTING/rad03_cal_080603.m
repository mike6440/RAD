...
pspampgain = [125,35]';
v_in = [0, 2, 10]'; % MV
adc_psp = [26.3,279.3,1271.7]';
% COMPUTE SW FROM INPUT VOLTAGE
sw_rad = [-1.3, 236.3, 1179.2]';   % sw output from RAD during test
...
fprintf('%10.4f %10.1f %10.1f %10.1f %10.1f\n', [v_in, adc_psp, sw_rad, sw_adc,sw_test]');
...
Rref = 33179;
% case
v_meas = [1.273, .948, .593]'*1000;  % VOLTAGE ACROSS TEST RESISTOR
% compute Rrefm
i_m = v_meas ./ r_test;
Rrefcal = (Vref - v_meas) ./ i_m;
% VOLTAGE AT ADC
ctest = [1327.0, 1004.8, 654.0]';
t_radout = [14.84, 24.32, 38.54]';
...
Rref = 33192;
% VOLTAGE AT ADC
ctest = [1322, 1001.75, 650.5]';
t_radout = [13.94, 23.21, 38.54]';
v_adc = Vref .* (ctest/2) ./ Cmax;
% CURRENT based on adc count
i = (Vref - v_adc) / Rref;
% THERMISTOR RESISTANCE, COMPUTED FROM ADC
