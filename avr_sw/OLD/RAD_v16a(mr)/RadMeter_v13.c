...
// v14x :: edited by Reynolds, see 'v14x'
/*********************************************
...
int state;
unsigned char version[10] = "1.13";
unsigned char verdate[20] = "2007/02/20"; 
double Tabs = 273.15;
int adc[NCHANS];   
...
// v14x -- note this offset is in mv as it is subtracted from the 
...
void main(void)
{
	...
	// v14x, the end of an output line will be <cr><lf>
	//Heartbeat();
	//*** why no heartbeat? 
	...
	while (1)
	{
		...
		//CALCULATE LW (PIR)
		// v14x, Note the count from the max186 is used for volts.
...
//              printf("then= %d, now= %d\n\r", then, now);
		...
		rtc_get_time(&h,&m,&s);
		now = s;
		//*** I am not sure.  s = 0,1,2,...,59,0,1,2... What if then = 59 and now = 2?
		//*** shouldn't you use the time() subroutine?
		//
		// example--
		//time_t then;
		//
		//then = time();
		//if( time() - then >= looptime )  {
		//	...
		//	then = time();
		//	...
		//}
		...
		{   
			...
			printf("$RAD%02d,%02d/%02d/%02d,%02d:%02d:%02d,%4d,%4d,%6.2f,%6.2f,%6.2f,%7.2f,%5.1f,%5.1f\n\r", 
		...
		//*** Ray did you add this recently, since Norfolk?
	}; //*** why the semicolon?  This is probably unnecessary 
} 
...
{ 
	...
	printf("\n\r");
	printf("\n\r NOAA RADIOMETER INTERFACE BOARD VERSION: %s, VERSION DATE: %s\n\r", version, verdate);
	...
	switch (ch) 
	{
		...
		case 'l' : 
			...
			if(atof(msg) > 3600 || atof(msg) <= 0)    //v13 increased limit to 1/2 hour // v14x increased to one hour
			{
				printf("\n\rOut of Range.\n\r");
				break;
			}
		...
		case 'p' :
			...
			if(atof(msg) >= 20.0E-6 || atof(msg) <= 0.1E-6)  // v14x  expand range
			{
				printf("\n\rOut of Range\n\r");
				break;
			}
		...
		case 'g' :
			...
			if(atof(msg) >= 300 || atof(msg) <= 10)  // v14x  expand range 
			{
				printf("\n\rOut of Range\n\r");
				break;
			}
		...
		case 'o' :
			...
			if(atof(msg) > 500 || atof(msg) < -500)  // v14x  expand range
			{
				printf("\n\rOut of Range\n\r");
				break;
			}
		...
		case 'G' :
			...
			if(atof(msg) > 1500 || atof(msg) < 500 )  // v14x  expand range
			{
				printf("\n\rOut of Range\n\r");
				break;
			}
		...
		case 'O' :
			...
			if(atof(msg) > 200 || atof(msg) < -200)  // v14x  expand range
			{
				printf("\n\rOut of Range\n\r");
				break;
			}
		...
		case 'V' :
			...
			if(atof(msg) > 6.0 || atof(msg) < 2.0)  // v14x  expand range
			{
				printf("\n\rOut of Range\n\r");
				break;
			}
		...
		case 'z' :
			...
			printf(" PSP ADC Gain= %.2f\n\r", PSPadc_gain);
			printf("\n\r");
			// v14x -- we need to wait here for a character press
			break;
